...
const selectionType = {
  ...
  const getCTST=() => {
    ...
    CTST.then((val)=>{
      ...
      let transformedData = data[0]?.value.map((item) => ({
        ...item,
        label: item.mstDesc,
        value: item.mstID
      }));
  ...
  const filterOption = (input, option) =>
    ...
    const getAdminData = async () => {
      //setIsLoading(true);
      let response = apiCalls.GetSerReqStatus(4);
      response
        .then((val) => {
          if (val?.data) {
        ...
        >
          <Row gutter={[16,16]} className='dashboard-filters'>
            ...
            <Col xs={12} sm={12} md={3} lg={3} xxl={3}>
              ...
              <span className="editIcon"> <i  onClick={() => saveAssignTo()} className="bi bi-send"></i></span>
              </span> */}
            </Col>
...
export default PosAdminDashboard