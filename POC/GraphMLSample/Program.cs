...
//doc.Load("PoC.graphml");

//XmlNode? node = doc.GetElementById("n1");

var fileName = "PoC.graphml"; // Replace with your GraphML file name
...
DataTable dataTable = commonService.GetDataTable();

// Graph Logic

DataSet ds = commonService.GraphFlow();
...
foreach(var nodes in graphmL!.Graph.Nodes)
{
    ...
    {
        ExecuteExpression(nodes);
    }
}

void ExecuteExpression(Node nodes) {
    ...
    {
        var data = dataTable.AsEnumerable().Where(f => f.Field<string>("SrvReqRefNo") == serviceReqId).FirstOrDefault();
        if (data != null)
        {
